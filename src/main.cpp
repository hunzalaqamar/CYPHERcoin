...
#if defined(NDEBUG)
# error "Hunzalacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x9a1f9323d579f7dd3957dfa273a83f0d3ee3462fedf21385bfb012fd4c1a535c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Hunzalacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Hunzalacoin Signed Message:\n";
...
{
    // Hunzalacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 47 * COIN;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 7800000); // Hunzalacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2.5 * 24 * 60 * 60; // Hunzalacoin: 2.5 days
static const int64 nTargetSpacing = 1.5 * 60; // Hunzalacoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Hunzalacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Hunzalacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe0bca66835d2f1601d7080651d84402617ec1628146da5b8a3655dc61e1aa7c7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "11/12/2022 - Hunzala Made his first coin and caught the culprit using web";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 47 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04dc7fa6fc7447b85072b9c990de7ab79f9a01e42234293b655b038d2c068cc12fd06ea97a0a24817176931d7a4f6f98ea22ed5eee470cf08e10d969d0ad46d49f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1670777761;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084696580;

        if (fTestNet)
        {
            block.nTime    = 1670777761;
            block.nNonce   = 386055601;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x808bfb5c0c17738a0b6a1c3e38cce75d2e41237a579563d35a514b5a459c1763"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc4, 0xb7, 0xdl }; // Hunzalacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HunzalacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HunzalacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HunzalacoinMiner(CWallet *pwallet)
{
    printf("HunzalacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hunzalacoin-miner");
    ...
    try { loop {
        ...
        printf("Running HunzalacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HunzalacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HunzalacoinMiner, pwallet));
}
